...
import com.dracul.feature_main.event.MainEffect
import com.dracul.feature_main.event.MainEffect.ShareNote
...
class MainComponent(
    ...
    private val onEditTask: (id: Long?) -> Unit,
    ...
    private val _events = MutableSharedFlow<MainEffect>(1)
    ...
    fun onAction(action: MainAction) {
        when (action) {
            ...
            is MainAction.ShareNoteModal -> {
                selectedItemId?.let { id ->
                    coroutineScope.launch {
                        _events.emit(ShareNote(getNoteByIdUseCase(id)))
                    }
                }
            }
            ...
            is MainAction.CreateReminder -> {
                ...
                val inputData = Builder().putString(
                    "MESSAGE",
            ...
            MainAction.CreateTask -> onEditTask(null)
        }
    }
}